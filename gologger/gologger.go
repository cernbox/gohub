...
func GetLoggedHTTPHandler(filename string, h http.Handler) http.Handler {
	...
	return handlers.RecoveryHandler()(newLoggingHandler(file, h))
}
...
// loggingHandler is the http.Handler implementation for newLoggingHandlerTo and its
// friends
...
func buildCommonLogLine(req *http.Request, url url.URL, ts time.Time, status int, size int) []byte {
	username := "-"
	// is basic auth is provided, the username is logged
	if name, _, _ := req.BasicAuth(); name != "" {
		username = name
	}
